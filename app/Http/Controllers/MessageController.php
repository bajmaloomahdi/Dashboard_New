...
class MessageController extends Controller
{
    public function index(Request $request, string $mode = 'inbox')
    {
        ...
        $messages = DB::select(
            'EXEC sp_GetMessages @UserID = ?, @Mode = ?, @IsArchive = 0, @SearchText = ?, @MessageTypeID = ?, @MessageStatusID = ?',
            [
                $userId,
                $modeValue,
                $searchText ?: null,
    ...
    public function archive(Request $request)
    {
        ...
        $messages = DB::select(
            'EXEC sp_GetMessages @UserID = ?, @Mode = 1, @IsArchive = 1, @SearchText = ?, @MessageTypeID = ?, @MessageStatusID = ?',
            [
                $userId,
                $searchText ?: null,
                $messageTypeId !== null && $messageTypeId !== '' ? (int) $messageTypeId : null,
